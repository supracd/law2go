...
class PromotionsModelTests(PromotionsTestBase):
    def test_banner_group_create(self):
        self.assertEqual(self.group_1.__unicode__(), self.group_1.slug)

    def test_banner_create(self):
        self.assertEqual(self.banner_1.__unicode__(), self.banner_1.title)
...
class PromotionsTemplateTagTests(PromotionsTestBase):
    def test_get_banner_by_group_tag(self):
        ...
        {% get_banners_by_group group_slug 2 as banners %}
        {% for b in banners %}
             {{b.title}}<br/>
        {% endfor %}
    ...
    def test_get_random_banner_by_group(self):
        output = Template("""
        {% load promotions_tags %}
        {% get_random_banner_by_group group_slug as banner %}
          {{banner.title}}<br/>
