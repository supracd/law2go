...
class PostModelTest(BlogTestBase):
    def test_post_create(self):
        ...
        self.assertEquals(
            self.post_1.__unicode__(),
            "{0} - {1}".format(self.post_1.title, self.post_1.publish_date))

    def test_category_create(self):
        self.assertEquals(
            self.category.__unicode__(), self.category.name)
...
class BlogTagsTests(BlogTestBase):
    ...
    def test_get_latest_public_posts(self):
        tpl = Template("""
           {% load blog_tags %}
           {% get_latest_public_posts 10 as posts %}
        """)
        context = Context()
        tpl.render(context)
    ...
    def test_get_used_public_categories(self):
        tpl = Template("""
           {% load blog_tags %}
           {% get_used_public_categories as cats %}
        """)
        context = Context()
        tpl.render(context)
        self.assertQuerysetEqual(context['cats'],
