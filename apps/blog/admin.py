...
class PostAdmin(admin.ModelAdmin):
    ...
    list_filter = ('publish_date', 'categories', 'status', 'is_featured')
    ...
    def make_published(self, request, queryset):
        rows_updated = queryset.update(status=Post.PUBLISHED_STATUS)
        if rows_updated == 1:
    ...
    def make_draft(self, request, queryset):
        rows_updated = queryset.update(status=Post.DRAFT_STATUS)
        if rows_updated == 1:
            message_bit = "1 object was"
        else:
            message_bit = "%s objects were" % rows_updated
